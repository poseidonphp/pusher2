...
type ServerConfig struct {
	...
	DispatchManager          dispatcher.DispatcherContract
	PubSubDriver             string
...
func InitializeServerConfig(ctx *context.Context, flags CommandLineFlags) (*ServerConfig, error) {
	...
	dispatchDriver := env.GetString("DISPATCHER_MANAGER", "local")
	...
	newConfig := &ServerConfig{
		...
		UsingRedis:               dispatchDriver == "redis" || pubSubDriver == "redis" || storageDriver == "redis" || channelCacheDriver == "redis",
...
func (s *ServerConfig) InitializeDispatcher() error {
	switch s.DispatchDriver {
	case "local":
		log.Logger().Debugln("Dispatcher driver is local")
		s.DispatchManager = &dispatcher.SyncDispatcher{WebhookManager: s.WebhookManager}
	case "redis":
		log.Logger().Debugln("Dispatcher driver is Redis")
		s.DispatchManager = &dispatcher.RedisDispatcher{
			RedisClient:    s.RedisInstance,
			WebhookManager: s.WebhookManager,
		}
	default:
		return errors.New("invalid dispatcher manager: " + s.DispatchDriver)
	}

	err := s.DispatchManager.Init()
	if err != nil {
		return errors.New(fmt.Sprintf("failed to initialize dispatcher: %v", err.Error()))
	}

	s.DispatchManager.InitFlapDetection(s.WebhookEnabled, s.DispatchManager, env.GetInt("FLAP_DETECTION_THRESHOLD", 3))

	s.GlobalWaitGroup.Add(1)
	go func() {
		defer s.GlobalWaitGroup.Done()
		s.DispatchManager.ListenForEvents(*s.ctx)
	}()
	return nil
}
